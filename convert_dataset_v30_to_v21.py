...
def convert_info(
    ...
    total_episodes = info.total_episodes or len(episode_records)
    chunks_size = info.chunks_size

    # 直接修改属性
    info.codebase_version = V21
    info.data_path = LEGACY_DATA_PATH_TEMPLATE
    info.video_path = LEGACY_VIDEO_PATH_TEMPLATE if len(video_keys) > 0 else None

    # 移除 v3 特有字段（如果存在）
    if hasattr(info, 'data_files_size_in_mb'):
        delattr(info, 'data_files_size_in_mb')
    if hasattr(info, 'video_files_size_in_mb'):
        delattr(info, 'video_files_size_in_mb')

    # 清理 features 中的 fps（仅视频保留）
